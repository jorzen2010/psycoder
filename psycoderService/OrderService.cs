...
namespace psycoderService
{
    public class OrderService
    {
        ...
        public static string GetFensiOrderActionById(int oid)
        {
            UnitOfWork unitOfWork = new UnitOfWork();
            FensiOrders order = unitOfWork.fensiOrdersRepository.GetByID(oid);
            string ac = "jinyong";
            string btnname = "操作";
            string btnstyle = "success";
            switch (order.Status)
        ...
        public static DateTime GetFensiOrderExpiryTimeByPid(int pid)
        {
            UnitOfWork unitOfWork = new UnitOfWork();
            DateTime ExpiryTime = DateTime.Now;
            var orders = unitOfWork.fensiOrdersRepository.Get(filter: u => u.Customer == pid, orderBy: q => q.OrderByDescending(u => u.Id));
            if (orders.Count() > 0)
            {
                FensiOrders order = orders.First();
                ExpiryTime = order.ExpiryTime;
